...
function reverseUpcaseString(inputString) {
  const upcasedString = inputString.toUpperCase();
  const reversedUpcasedString = reverseTheString(upcasedString);
  return reversedUpcasedString;
}
function reverseTheString(inStr) {
  const inStrArr = inStr.split("");
  const inStrArrRev = inStrArr.reverse();
  let revString = "";
  inStrArrRev.forEach(function (char) {
    revString += char;
  });
  return revString;
}
...
function removeEnds(inString) {
  ...
  if (inStringArr.length < 3) {
    return "";
  } else {
    const removedArr = removeFirstAndLastChars(inStringArr);
    const removedString = returnToString(removedArr);
    return removedString;
  }
}
function removeFirstAndLastChars(inputArr) {
  inputArr.pop();
  inputArr.shift();
  return inputArr;
}
function returnToString(modifiedArr) {
  modifiedString = "";
  modifiedArr.forEach(function (char) {
    modifiedString += char;
  });
